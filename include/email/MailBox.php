...
class Mailbox {
	...
	/**
         ...
         * @throws Exception
         */
	public function __construct($imapPath, $login, $password, $attachmentsDir = null, $port = 143, $service = 'imap', $serverEncoding = 'UTF-8') {
		$this->port = $port;
		$this->service = $service;
		$this->setImapPath($imapPath, $port, $service);
	 ...
	 * @param string $imapPath
	 */
	public function switchMailbox($folder = '') {
		
		$imapStream = @imap_reopen($this->getImapStream(), $this->imapPath.$folder);
		if(!$imapStream) {
    ...
    /**
     * Get mail data
     * Mark as seen can be made true or false, depending upon we want the mails to be read or not when we read it for the first time
     *
     * @param $mailId
     * @param bool $markAsSeen
     * @return IncomingMail
     */
	public function getMail($mailId, $markAsSeen = false) {
		...
		$mail->unseen = $head->Unseen;
		$mail->flagged = $head->Flagged;
		$mail->answered = $head->Answered;
		$mail->deleted = $head->Deleted;
	...
	protected function initMailPart(IncomingMail $mail, $partStructure, $partNum, $markAsSeen = true) {
		...
		if($attachmentId) {
			...
			if($this->attachmentsDir) {
				...
				$fileSysName = time().'_'.preg_replace('~[\\\\/]~', '', $mail->id . '_' . $attachmentId . '_' . preg_replace(array_keys($replace), $replace, $fileName));
	...
	protected function setImapPath($imapPath, $port)
	{
		// todo: Imap Header autocreate
		
		switch ($port) {
            case 143:
                $this->setFlagCustom('novalidate-cert');
                break;

            case 993:
                $this->setFlagCustom('ssl');
                break;
        }

        $mailboxPath = '{' . $imapPath;
		...
		if (function_exists('mb_convert_encoding')) {
			$this->imapPath = mb_convert_encoding($mailboxPath, "UTF7-IMAP", "UTF-8");
		} else {
			$this->imapPath = imap_utf7_encode($mailboxPath);
		}
	}

	public function setFlagCustom($flag, $value = null)
    {
